<?php
 ...
 <?php
  ...
  $yearE = $_POST['lstYearE'];
  $query = "SELECT * FROM localapplicant JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo and applicant.entryYear='$yearE' ";
   ...
   function filterQuery($year,$title)
  {
	...
	// (1,_,_)
		if ($year<>'0')
		{
			...
			if ($title<>'0')
			{
				$subQuery = $subQuery." AND titleE='".$title."' AND code IS NOT NULL AND appType='Local' "; 
				
				 }// (1,1,1)
				 else if ($title=='0'){
					$subQuery = " WHERE alYear='".$year."' AND code IS NOT NULL AND appType='Local'"; 
				}
	...
	if ($year=='0')
		{
			...
			if ($title<>'0')
			{
				$subQuery = " WHERE titleE='".$title."' AND code IS NOT NULL AND appType='Local'"; 
...
?>
  ...
  <table style="margin-left:8px" class="panel">
      ...
      <td> Year of Entry</td>
        <td>
            <select name="lstYearE" id="lstYearE" onchange="this.form.submit();">
            <?php
				...
				//2021-03-23 end	
				{
					...
					//2021-03-23 end
					{
						if (isset($_POST['lstYearE']) && $_POST['lstYearE']==$row['entryYear'])
							echo "<option selected='selected' value='".$row['entryYear']."'>".$row['entryYear']."</option>";
						else if (isset($_SESSION['alYear']) && $_SESSION['alYear']==$row['alYear'])
							echo "<option selected='selected' value='".$row['entryYear']."'>".$row['entryYear']."</option>";
						else echo "<option value='".$row['entryYear']."'>".$row['entryYear']."</option>";
					}
				}
			?>
            </select>
...
</th><th>AppNo</th><th>Name</th><th ></th>    </tr>
<script>
