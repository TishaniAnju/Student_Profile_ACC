...
$lateType = $db->cleanInput($_GET['lateType']);
...
<h4 align="center"><font face="kaputadotcom2004" size="+2"><?php echo $ALYear ?> According to the  GCE A / L results</font></h4>
    ...
    <?php 
	if ($lateType=='y'){
	$result = $db->executeQuery("SELECT * FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE entryYear = '$entryYear' AND alYear = '$ALYear' AND lateApp='y' ORDER BY zScore");
		//$test="SELECT * FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE entryYear = '$entryYear' AND alYear = '$ALYear' AND lateApp='y' ORDER BY zScore";
	}
	 else{
		 
		 $result = $db->executeQuery("SELECT * FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE entryYear = '$entryYear' AND alYear = '$ALYear' AND lateApp!='y' ORDER BY zScore");
	 }
	// echo "SELECT * FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE entryYear = '$entryYear' AND alYear = '$ALYear' AND lateApp!='y' ORDER BY zScore";
	// echo "SELECT * FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE entryYear = '$entryYear' AND alYear = '$ALYear' AND lateApp=='y' ORDER BY zScore";
