...
<?php
	...
	<?php
		...
		$result1 = $db->cleanInput($_POST['lstResult1']);
		$result2 = $db->cleanInput($_POST['lstResult2']);
		$result3 = $db->cleanInput($_POST['lstResult3']);
		$code = $_POST['lstPali'];
			...
			?>
			  ...
			  <tr>          
				...
				<td><select name="lstPali[]" id="lstPali">
				  <?php
			if (mysqli_num_rows($resultPali)>0)
			{				 		
...
<?php
	...
	{
		$appNo = $db->cleanInput($_POST['txtAppNo']);
		$nicNo =$db-> cleanInput($_POST['txtppNo']);
		...
		$subject = $db->cleanInput($_POST['txtsubjects']);
	...
	//$result = executeQuery($query);
	$result = $db->executeQuery($query);	//$row = mysql_fetch_array($result);
