...
<form action="" method="post">
    <table><tr>
            ...
            <td><select name="indexNo" id="indexNo" required="true">
                    ...
                    <?php
                    //2021.03.24 start  $result = executeQuery("SELECT distinct`indexNo` FROM `studentenrolment`");
                    $result = $db->executeQuery("SELECT distinct`indexNo` FROM `studentenrolment`");
                    //2021.03.24 end
        ...
        <tr>
            ...
            <td><select name="level" id="level" required="true">
                    <?php
                    //2021.03.24 start  $result = executeQuery("SELECT distinct `level` FROM `subject` ");
        ...
        <tr>
            ...
            <td><select name="semester" id="semester" required="true">
                    <?php
                    //2021.03.24 start  $result = executeQuery("SELECT distinct `semester` FROM `subject` ");
    ...
    if (isset($_POST['submit'])) {
        ...
        //2021.03.24 start  $result = executeQuery("SELECT x.subjectID,s.codeEnglish,s.nameEnglish,x.grade FROM `exameffort` x,`subject` s WHERE x.indexNo='$indexNo' and x.acYear='$year' and s.semester='$semester' and  x.subjectID = s.subjectID");
        $result = $db->executeQuery("SELECT x.subjectID,s.codeEnglish,s.nameEnglish,x.grade FROM `exameffort` x,`subject` s WHERE x.indexNo='$indexNo' and x.acYear='$year' and s.semester='$semester' and  x.subjectID = s.subjectID");
        //2021.03.24 end
        //2021.03.24 start  while ($row = mysql_fetch_array($result))
        while ($row=$db->Next_Record($result)) 
        {
...
//    echo '</table>';
        ...
        //2021.03.24 start  $result4 = executeQuery("select e.subjectID,s.creditHours from `studentenrolment` e,`subject` s where e.indexNo='$indexNo' and e.subjectID=s.subjectID and s.level='$level'and s.semester='$semester'");
        $result4 = $db->executeQuery("select e.subjectID,s.creditHours from `studentenrolment` e,`subject` s where e.indexNo='$indexNo' and e.subjectID=s.subjectID and s.level='$level'and s.semester='$semester'");
	    //2021.03.24 end
        echo '<tr><td>Subject ID</td><td>Credit Hours(CH)</td><td>Grade Point(GP)</td><td>CH * GP</td><td>sum CH</td><td>sum GP</td></tr>';
        //2021.03.24 start  while ($row = mysql_fetch_array($result4)) 
        while ($row=$db->Next_Record($result4))
        {
            //calculate total credit hours//
            $sumCreditaHours = $sumCreditaHours + $row[1];

            //2021.03.24 start  $result3 = executeQuery("SELECT `gradePoint`,`effort` FROM `exameffort` WHERE `indexNo`='$indexNo' and subjectID='$row[0]' order by effort desc limit 1");
            $result3 = $db->executeQuery("SELECT `gradePoint`,`effort` FROM `exameffort` WHERE `indexNo`='$indexNo' and subjectID='$row[0]' order by effort desc limit 1");
            //2021.03.24 end
      
			//2021.03.24 start  $row3 = mysql_fetch_array($result3);
        ...
        //2021.03.24 start  $value = executeQuery("SELECT * FROM `studentgpa` WHERE `indexNo`= '$indexNo' and `acYear`='$year' and `level`='$level' and `semester`='$semester'");
        $valuequary = "SELECT * FROM `studentgpa` WHERE `indexNo`= '$indexNo' and `acYear`='$year' and `level`='$level' and `semester`='$semester'";
	//print $valuequary;
		 $resultquary = $db->executeQuery($valuequary);
		...
		print $numRows; 
		print "llll";
		$db->Row_Count($value)==0 */
///////////////////////////////////////////////////////////////Insert and Update values///////////////////
        if ($db->Row_Count($resultquary)==0) {
            //2021.03.24 start  executeQuery("INSERT INTO `studentgpa`(`indexNo`, `acYear`, `level`, `semester`,`totcredithours`, `totGPA`, `GPAsemester`, `Acount`, `Bcount`, `Ccount`, `TotalSub`) VALUES ('$indexNo','$year','$level','$semester','$sumCreditaHours','$sumGradePoint',' $semesterGpa','$Aorabove','$Borabove','$Corabove','$totalSubjects')");
            $db->executeQuery("INSERT INTO `studentgpa`(`indexNo`, `acYear`, `level`, `semester`,`totcredithours`, `totGPA`, `GPAsemester`, `Acount`, `Bcount`, `Ccount`, `TotalSub`) VALUES ('$indexNo','$year','$level','$semester','$sumCreditaHours','$sumGradePoint',' $semesterGpa','$Aorabove','$Borabove','$Corabove','$totalSubjects')");
			 print "INSERT INTO `studentgpa`(`indexNo`, `acYear`, `level`, `semester`,`totcredithours`, `totGPA`, `GPAsemester`, `Acount`, `Bcount`, `Ccount`, `TotalSub`) VALUES ('$indexNo','$year','$level','$semester','$sumCreditaHours','$sumGradePoint',' $semesterGpa','$Aorabove','$Borabove','$Corabove','$totalSubjects')";
			//print  'kkkk';
            //2021.03.24 end
        } else if ($db->Row_Count($resultquary) > 0) {
            //2021.03.24 start  executeQuery("UPDATE `studentgpa` SET `totcredithours`='$sumCreditaHours',`totGPA`='$sumGradePoint',`GPAsemester`='$semesterGpa' , `Acount`='$Aorabove', `Bcount`='$Borabove', `Ccount`='$Corabove', `TotalSub`='$totalSubjects' WHERE `indexNo`='$indexNo' and `acYear`='$year' and `level`='$level' and `semester`='$semester'");
            $db->executeQuery("UPDATE `studentgpa` SET `totcredithours`='$sumCreditaHours',`totGPA`='$sumGradePoint',`GPAsemester`='$semesterGpa' , `Acount`='$Aorabove', `Bcount`='$Borabove', `Ccount`='$Corabove', `TotalSub`='$totalSubjects' WHERE `indexNo`='$indexNo' and `acYear`='$year' and `level`='$level' and `semester`='$semester'");
            //2021.03.24 end
			//print  'ttt';
...
<form method="post" action="" onsubmit="return validate_form(this);" class="plain">
    ...
    if (isset($_POST['finalGPA'])) {
        ...
        //2021.03.24 start  $valueFinal = executeQuery("SELECT `level`,`semester`,`totcredithours` ,`totGPA`,`GPAsemester`, `Acount`, `Bcount`, `Ccount`, `TotalSub` FROM `studentgpa` WHERE `indexNo`='$indexNo' order by level ");
        $valueFinal = $db->executeQuery("SELECT `level`,`semester`,`totcredithours` ,`totGPA`,`GPAsemester`, `Acount`, `Bcount`, `Ccount`, `TotalSub` FROM `studentgpa` WHERE `indexNo`='$indexNo' order by level ");
        //2021.03.24 end
        //2021.03.24 start  if (mysql_num_rows($result)==0) 
        if ($db->Row_Count($valueFinal)==0) 
        //2021.03.24 end
        {
            die("The student with index number $indexNo has no details");
        }
        ...
        //2021.03.24 start  while ($row = mysql_fetch_array($valueFinal)) 
        while ($row = $db->Next_Record($valueFinal)) 
        {
            $count = $count + 1;
