...
<?php 
	...
	$regNo = $db->cleanInput($_GET['lstRegNo']);
	
	$result = $db->executeQuery("SELECT * FROM student WHERE regNo='$regNo'");
...
?>
	...
	<table align="center" width="80%">
        ...
        <tr><td valign="top"><b>Enrolled Subjects:</b></td>
        	<td>
<?php
		$result = $db->executeQuery("SELECT codeEnglish, nameEnglish, codeSinhala, nameSinhala FROM studentenrolment JOIN subject ON studentenrolment.subjectID=subject.subjectID WHERE regNo='".$row['regNo']."'");
		//while ($row = mysql_fetch_array($result))
        while($row = $db->Next_Record($result))
		{
			echo $row['codeEnglish']." - ".$row['nameEnglish'].'  ('.$row['nameSinhala'].')'."<br/>";
		}
?>         		
            </td>
        </tr>
