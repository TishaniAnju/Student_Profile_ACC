<?php
 ...
 /// include("authcheck.inc.php");
  ...
  if(isset($_GET['page'])) $pageNum = $_GET['page'];
	...
	$query = "SELECT interview_list.appNo,id,title,name,qualified,confirmed,applicant.applicationNo FROM interview_list JOIN applicant ON interview_list.appNo=applicant.appNo ORDER BY id";
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST')
  {
	...
	$query = "SELECT interview_list.appNo,id,title,name,qualified,confirmed,applicant.applicationNo FROM interview_list JOIN applicant ON interview_list.appNo=applicant.appNo  and applicant.entryYear='$entryYear'  ORDER BY id";
		...
		if (isset($_POST['btnDeselect'])) 
		{
			...
			if ($Count>0)
			{
				foreach ($rowNum as $a)
					...
					$query1 = "DELETE FROM interview_list WHERE appNo='$row[0]'";
...
// counting the offset
	...
	//2021.03.24 end
?>
    <h1>Generating Final Qualified List</h1>
    <div id="tabs">
      <ul>
...
<form method="post" action="studentConfirm.php" class="plain" form name="appicant">
  <table class="panel" style="margin-left:8px">
    <tr>
    	 <td><input name="btnNew" type="button" value="Back" onclick="document.location.href = 'interview_list.php';" class="button" />
...
?>
 ...
 <table width="515" class="panel" style="margin-left:8px">
    <tr> 
	  <td width="140"><input name="btnSelect2" type="submit" id="btnSelect2" value="Generate Final Qualified List" class="button"/> 
