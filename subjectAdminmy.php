<?php
 ...
 <?php
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST')
  {
	...
	$subQuery = filterQuery($faculty,$level,$semester);
 ...
 /*if ($spCategory<>0){
				$query = $query." AND spCategory='".spCategory."'";
				} // (0,1,1)
  else{
  $query = $query;
   }*/
   $query = $query." ORDER BY level,semester,suborder";  
   print $query;
 /* 
  ...
  */
 function filterQuery($faculty,$level,$semester)
  {
	$subQuery = "";
	//print $faculty;
	if ($faculty<>'0')
	{
		$subQuery = " WHERE faculty='".$faculty."'"; // (1,_,_)
		
		if ($level<>'0')
		{
			$subQuery = $subQuery." AND level='".$level."'"; // (1,1,_)
			if ($semester<>'0')
				$subQuery = $subQuery." AND semester='".$semester."'"; // (1,1,1)
		}
		else if ($semester<>'0')
			$subQuery = $subQuery." AND semester='".$semester."'"; // (1,0,1)
	}
	else
	{
		if ($level<>'0')
		{
			...
			if ($semester<>'0')
				$subQuery = $subQuery." AND semester='".$semester."'"; // (0,1,1)
		}
		else if ($semester<>0)
			$subQuery = " WHERE semester='".$semester."'"; // (0,0,1)
	}
  ...
  //$query=$query." ORDER BY level,suborder ASC";
  // counting the offset
  ...
  <table style="margin-left:8px" class="panel">
  	<tr>
        ...
        <td>
            ...
            <?php
				$result = executeQuery("SELECT DISTINCT spCategory FROM subject");
				if (mysql_num_rows($result)>0)
				{
					echo "<option value='0'>All</option>";
					while ($row=mysql_fetch_array($result))
					{
						if (isset($_POST['lstsubject']) && $_POST['lstsubject']==$row['spCategory'])
							echo "<option selected='selected' value='".$row['spCategory']."'>".$row['spCategory']." </option>";
						else if (isset($_SESSION['lstsubject']) && $_SESSION['lstsubject']==$row['spCategory'])
							echo "<option selected='selected' value='".$row['spCategory']."'>".$row['spCategory']."</option>";
						else echo "<option value='".$row['spCategory']."'>".$row['spCategory']."</option>";
					}
				}
			?>
            </select>
