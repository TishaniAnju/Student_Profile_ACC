...
<h3 align="center"><font size="+2"> අයඳුම් කර ඇති සිසුන්ගේ නාම ලේඛනය(පාලි සුදුසුකම් යටතේ සමත්) </font></h3>
<h4 align="center"><font face="kaputadotcom2004" size="+2"><?php echo $ALYear ?> අ.පො.ස උ.පෙළ ප්‍රතිඵල අනුව</font></h4>
    ...
    <?php 
	if($title == 'All'){
	$result = $db->executeQuery("SELECT *,cast(zScore AS DECIMAL(10,4)) as 'valueee' FROM localapplicant JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE alYear = '$ALYear' AND entryYear='$entryYear' AND code IS NOT NULL  ORDER BY valueee DESC
    ");
	$i =1; 
	...
	//while ($row = mysql_fetch_array($result))
		...
		if($title != All)
		{
			$result = $db->executeQuery("SELECT *, cast(zScore AS DECIMAL(10,4)) as 'valueee'  FROM localapplicant JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE alYear = '$ALYear' AND entryYear='$entryYear' AND code IS NOT NULL AND titleE = '$title'  ORDER BY valueee DESC
    ");
	$i =1; 
	while($row = $db->Next_Record($result))
