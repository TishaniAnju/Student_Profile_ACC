...
<h4 align="center"><font face="kaputadotcom2004" size="+2">Pali Qualification (<?php if($title == 'Ven.'){echo "Monks";}elseif($title=='Mr.'){echo "Boys";}elseif($title=='All'){echo "All";}?>) </font></h4>
	...
	{
	//$result = $db->executeQuery("SELECT * FROM localapplicant JOIN applicantsubjects ON localapplicant.appNo = applicantsubjects.appNo WHERE alYear = '$ALYear' AND subjectCode = '12'   ORDER BY zScore");
    $result = $db->executeQuery("SELECT * FROM localapplicant JOIN applicantsubjects ON localapplicant.appNo = applicantsubjects.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE alYear = '$ALYear' AND subjectCode = '12'  ORDER BY zScore DESC");
	$i =1; 
