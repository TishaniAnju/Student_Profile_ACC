 ...
 /* if (isset($_POST['txtSearch']) && strlen($_POST['txtSearch'])>0)
  {
	...
	//2021.03.24 end
	$query="SELECT applicant.appNo,titleE,nameEnglish,appType,qualified,entryType,zScore,subjectCode,result FROM applicant JOIN localapplicant ON localapplicant.appNo = applicant.appNo JOIN applicantsubjects ON applicantsubjects.appNo = applicant.appNo   WHERE applicantsubjects.subjectCode = '4' AND appType='Local' AND entryYear = '$year[0]' ORDER BY localapplicant.zScore DESC";
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
  {
	...
	$query="SELECT applicant.appNo,titleE,nameEnglish,appType,qualified,entryType,zScore,subjectCode,result FROM applicant JOIN localapplicant ON localapplicant.appNo = applicant.appNo JOIN applicantsubjects ON applicantsubjects.appNo = applicant.appNo WHERE applicantsubjects.subjectCode = '4' AND  appType='Local' AND entryYear = '$entryYear' ORDER BY localapplicant.zScore DESC";
