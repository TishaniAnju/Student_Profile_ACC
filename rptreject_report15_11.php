...
<h4 align="center"><font face="kaputadotcom2004" size="+2">Disqualified Student List (<?php if($title == 'Ven.'){echo "Monks";}elseif($title=='Mr.'){echo "Boys";}elseif($title=='All'){echo "All";}elseif($title=='Ms.'){echo "Girls";}elseif($title=='Mrs.'){echo "Girls";}?>)</font></h4>
</head>
    ...
    localapplicant.appNo NOT IN ( SELECT localapplicant.appNo FROM localapplicant JOIN applicantsubjects ON localapplicant.appNo = applicantsubjects.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE alYear = '$ALYear' AND subjectCode = '12') 
    AND localapplicant.appNo NOT IN (SELECT localapplicant.appNo FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo WHERE result IS NOT NULL AND alYear = '$ALYear' ) 
    AND alYear = '$ALYear' AND entryYear='$entryYear' ORDER BY zScore DESC");
	$i =1; 
	print "SELECT * FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE 
    localapplicant.appNo NOT IN ( SELECT localapplicant.appNo FROM localapplicant JOIN applicantsubjects ON localapplicant.appNo = applicantsubjects.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE alYear = '$ALYear' AND subjectCode = '12') 
    AND localapplicant.appNo NOT IN (SELECT localapplicant.appNo FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo WHERE result IS NOT NULL AND alYear = '$ALYear' ) 
    AND alYear = '$ALYear' AND entryYear='$entryYear' ORDER BY zScore DESC" ;
	...
	//while ($row = mysql_fetch_array($result))
		...
		if($title != 'All')
		{
            ...
            localapplicant.appNo NOT IN ( SELECT localapplicant.appNo FROM localapplicant JOIN applicantsubjects ON localapplicant.appNo = applicantsubjects.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE alYear = '$ALYear' AND subjectCode = '12') 
            AND localapplicant.appNo NOT IN (SELECT localapplicant.appNo FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo WHERE result IS NOT NULL AND alYear = '$ALYear' ) 
            AND alYear = '$ALYear' AND titleE='$title'  AND entryYear='$entryYear' ORDER BY zScore DESC");
	$i =1; 
	while($row = $db->Next_Record($result))
