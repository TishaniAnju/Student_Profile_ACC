<?php
 ...
 <?php
  ...
  $query = "SELECT * FROM student JOIN applicant ON applicant.appNo=student.appNo WHERE applicant.appType = 'Foreign' AND student.confirmed ='Yes'"  ; 

 // $query = "SELECT * FROM student where `confirmed`='Yes' "; // JOIN localapplicant ON student.appNo=localapplicant.appNo";  
...
<h1>Student Administration - Foreign Applicants</h1>
  ...
  <tr>
		...
		<td>&nbsp;</td>
		<td colspan="2" ><input name="btnDetails2" type="button" value="Local Students" onclick="document.location.href ='studentAdmin.php'" class="button" /></td>
    </tr>
    <tr>
        <td>Subject</td>
        <td>
        ...
        <td>
            ...
            <?php
				//2021.03.25 start  $result = executeQuery("SELECT DISTINCT faculty FROM student where faculty IS NOT NULL");
        ...
        <td>
            ...
            <?php
				//2021.03.25 start  $result = executeQuery("SELECT DISTINCT yearEntry FROM student");
...
<?php 
//2021.03.25 start  if (mysql_num_rows($pageResult)>0)
