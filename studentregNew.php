		...
		//echo $Count;
       	if ($Count>0){
			...
			//2021.03.24 end
				...
				$queryBS = "SELECT * FROM `student` WHERE `regNo` LIKE '%BS/2023'";
				$resultBS = $db->executeQuery($queryBS);
				$rowcountBS=mysqli_num_rows($resultBS);
				$queryLS = "SELECT * FROM `student` WHERE `regNo` LIKE '%LS/2023'";
				$resultLS = $db->executeQuery($queryLS);
				$rowcountLS=mysqli_num_rows($resultLS);
...
//2021.03.24 end
		...
		if (isset($_POST['btnSAselect'])) {
			$rowNum = $_POST['chk'];
		 $Count=count($rowNum);
		 $queryLSSA = "SELECT * FROM `student` WHERE `regNo` LIKE '%LS/SA/2023%'";
		 $resultLSSA = $db->executeQuery($queryLSSA);
		 $rowcountLSSA=mysqli_num_rows($resultLSSA);
...
<form method="post" action="studentreg.php" class="plain" form name="appicant">
