...
<h4 align="center"><font face="kaputadotcom2004" size="+2">Pali Qualification (<?php if($title == 'Ven.'){echo "Monks";}elseif($title=='Mr.'){echo "Boys";}elseif($title=='all'){echo "all";}?>) </font></h4>
</head>
	...
	if($title == 'all'){ 
		...
		$result = $db->executeQuery("SELECT *, cast(zScore AS DECIMAL(10,4)) as 'valueee' FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE 
            localapplicant.appNo NOT IN ( SELECT localapplicant.appNo FROM localapplicant JOIN applicantsubjects ON localapplicant.appNo = applicantsubjects.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE  subjectCode = '12') 
            AND localapplicant.appNo NOT IN (SELECT localapplicant.appNo FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo WHERE result IS NOT NULL  ) 
	...
	if($title != 'all')
		{
            ...
            localapplicant.appNo NOT IN ( SELECT localapplicant.appNo FROM localapplicant JOIN applicantsubjects ON localapplicant.appNo = applicantsubjects.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE  subjectCode = '12') 
            AND localapplicant.appNo NOT IN (SELECT localapplicant.appNo FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo WHERE result IS NOT NULL  ) 
             AND titleE='$title'  AND entryYear='$entryYear' ORDER BY valueee DESC");
	$i =1; 
	while($row = $db->Next_Record($result))
	...
	<tr>
			...
			<td align="center"><?php echo $row['applicationNo'];?></td>
			<?php 
			//$address = $row['addressSinhala1']."".$row['addressSinhala2']."".$row['addressSinhala3'];
			//$addressLength = strlen($address);
			//if ($addressLength<60)
			//echo "<td>".$address."</td>";
