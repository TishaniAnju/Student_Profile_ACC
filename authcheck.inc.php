...
if (!$notAuthenticated) 
{
	...
	$result = $db->executeQuery("SELECT * FROM privillege WHERE userID='$userID' AND pageID IN (SELECT pageID FROM page WHERE pageName='$currentPage')");
	if ($db->Row_Count($result)>0) $notPrivilleged = false;
}
