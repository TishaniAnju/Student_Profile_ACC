...
$filename ="rpt_pali_qualification.xls";
...
<h4 align="center"><font face="kaputadotcom2004" size="+2">Other Pali Qualification (<?php if($title == 'Ven.'){echo "Monks";}elseif($title=='Mr.'){echo "Boys";}elseif($title=='All'){echo "All";}elseif($title=='Ms.'){echo "Girls";}elseif($title=='Mrs.'){echo "Girls";}?>)</font></h4>
    ...
    <?php 
	if($title == 'All'){
	$result = $db->executeQuery("SELECT *, cast(zScore AS DECIMAL(10,4)) as 'valueee' FROM localapplicant JOIN applicantpali ON localapplicant.appNo = applicantpali.appNo JOIN applicant ON localapplicant.appNo = applicant.appNo WHERE alYear = '$ALYear' AND entryYear='$entryYear' AND result IS NOT NULL  ORDER BY  valueee DESC
    ");
	$i =1; 
