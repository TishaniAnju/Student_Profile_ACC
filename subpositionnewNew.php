...
if (isset($_GET['cmd']) && $_GET['cmd']=="delete")
  {
	...
	$positionsQuery = $db->executeQuery("SELECT position FROM positionsNew WHERE id='$effort'");

	while ($positionRow = $db->Next_Record($positionsQuery)) {
	...
	//2021-03-25 start  $result = executeQuery("Insert into exameffort (indexNo,subjectID,mark1,mark2,marks,grade,gradePoint,acYear,effort,medium)VALUES ('$indexNovalue','$SubjectID','$mark1','$mark2','$marks','$grade','$gradepoint','$acyear','$effort','$medium')");
			$result = $db->executeQuery("Insert into sub_positionNew (subId,positionId,position)VALUES ('$SubjectID','$effort','$positionId')");
...
<table class="searchResults">
   <tr>
    ...
    <td>Position : </td>
    <td>
        <select name="lstEffort" id="lstEffort" onchange="document.form1.submit()">
            <?php
            // Fetch positions from positionsNew table
            $positionsQuery = $db->executeQuery("SELECT position, p_name,id FROM positionsNew WHERE level='$level' and semester='$semester' order by position");
    ...
    <tr>
        ...
        <td><select name="lstSubject" id="lstSubject" style="width:auto" onchange="document.form1.submit()">
			...
			$query = "SELECT * FROM Newsubject WHERE faculty='$faculty' and level='$level' and semester='$semester'";
			//print $query;
    ...
    <?php
    $queryallsub = $db->executeQuery("SELECT * FROM Newsubject, sub_positionNew WHERE sub_positionNew.positionId='$effort' and Newsubject.level='$level' and Newsubject.semester='$semester' and Newsubject.subjectID=sub_positionNew.subId");
    ...
    while ($rowallsub = $db->Next_Record($queryallsub)) {
        ...
        <tr>
            ...
            <td><?php echo $rowallsub['position'] ?></td>
            <td><input name="btnDelete" type="button" value="Remove" class="button" onclick="if (MsgOkCancel()) document.location.href ='subpositionnewNew.php?cmd=delete&subjectID=<?php echo $rowallsub['subjectID'] ?>'" /></td>
        </tr>
    <?php
    }
    ?>
