 ...
 /* if (isset($_POST['txtSearch']) && strlen($_POST['txtSearch'])>0)
  {
  	...
  	$query = "SELECT * FROM gradepoints ORDER BY Id";
  
 if (isset($_GET['cmd']) && $_GET['cmd']=="delete")
	{
		//2021.03.24 start  $subCode = cleanInput($_GET['Id']);
    $subCode = $db->cleanInput($_GET['Id']);
    //2021.03.24 end
		//$subNS= cleanInput($_GET['subnamS']);
		$delQuery1 = "DELETE FROM gradepoints WHERE Id='$Id'";
...
<form method="post" action="" class="plain">
  <table style="margin-left:8px"><tr><td><input name="btnNew" type="button" value="New" onclick="document.location.href = 'newGrade.php';" class="button" /></td><td>&nbsp;&nbsp;&nbsp;</td></table>
	<?php 
  ...
  <table class="searchResults">
	<tr>
    	<th>Id</th><th>Lower Marks</th><th>Upper Marks</th><th>Geade</th><th>Points</th>
        <th colspan="2">&nbsp;</th>
	</tr>
...
?>
	<tr>
		...
		<td><input name="btnEdit" type="button" value="Edit" onclick="document.location.href ='editAlSub.php?Id=<?php echo $row['Id'] ?>'" class="button" /></td>
		<td><input name="btnDelete" type="button" value="Delete" onclick="if (MsgOkCancel()) document.location.href ='gradePoints.php?cmd=delete&Id=<?php echo $row['Id'] ?>';" class="button" /></td>
	</tr>
<?php
  }  
?>
...
<?php
  ...
  ob_end_clean();
  $pagetitle = "Grade Points - Student Management System - Buddhist & Pali University of Sri Lanka";
 $navpath = "<ul><li><a href='home.php'>Home </a></li><li>Grade Points</li></ul>";
  //Apply the template
