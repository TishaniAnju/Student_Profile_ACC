...
<table border="1" cellpadding="2" cellspacing="0">
    <thead>
            ...
            <tr>
                ...
                <th>Subject 3</th>
                <?php 
                // Display Pali Qualification headers
        ...
        <tbody>
            ...
            $result = $db->executeQuery("SELECT * FROM localapplicant JOIN applicant ON localapplicant.appNo = applicant.appNo JOIN chapter ON localapplicant.chapter = chapter.chapterID
                JOIN nikaya ON localapplicant.nikaya = nikaya.nikayaID  WHERE entryYear = '$entryYear' ORDER BY applicationNo");
            $i = 1; 
            ...
            while($row = $db->Next_Record($result)) {
                ...
                <tr>
                    ...
                    if($db->Row_Count($resultSub1) >= 1 && $db->Row_Count($resultSub1) <= 3) {
                        while($rowSub1 = $db->Next_Record($resultSub1))
                            echo "<td align='center'>".$rowSub1['subnameE']. ":" .$rowSub1['result']."</td>";
                        
                        // Add missing cells if subjects are less than 3
                        for ($j = 1; $j <= (3 - $db->Row_Count($resultSub1)); $j++) {
                            echo "<td> - </td>";
                        }
                    } else {
                        // Display empty cells if no subjects
                        for ($j = 1; $j <= 3; $j++) {
                            echo "<td> - </td>";
                        }
                    }

                    // Display Pali Qualification results
                    for ($j = 1; $j <= 6; $j++) {
